...
const NextScreenButton: FC<INextScreenText> = ({ text, link }) => {
    return (
        <>
            <Link to={`/${link}`} className='text-white/70 text-md font-light flex items-center'>
                <span className='pr-4 hover:underline transition-all duration-500 ease-in-out capitalize'>{text}</span>
